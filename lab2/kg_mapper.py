...
def main():
    ...
    KG_INPUT_PATH = 'data/kg_filtered.txt'  # 之前生成的知识图谱文件路径
    KG_OUTPUT_PATH = 'baseline/data/Douban/kg_final.txt'

    # 加载电影ID映射
    movie_index_map = load_movie_id_map(MOVIE_ID_MAP_PATH, DOUBAN_TO_FB_PATH)
    num_of_movies = max(movie_index_map.values()) + 1
    ...
    # 创建实体和关系的索引映射
    entity_id_map, relation_id_map = create_entity_relation_mappings(triple_list, movie_index_map)
    ...
    # 将三元组映射为索引表示
    mapped_triples = map_triples_to_ids(triple_list, movie_index_map, entity_id_map, relation_id_map)
